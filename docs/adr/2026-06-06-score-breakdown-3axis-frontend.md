...
## 追記（2026-06-06）: 軸ラベルを平易化（オーナー指示）

当初の「コンテンツ素地／技術素地／権威・エンティティ認知」は「素地」が非専門家（代理店のクライアント）に伝わりにくいとのフィードバックを受け、平易な説明型ラベル＋補足一文に変更した。配点・ロジックは不変（表示文言のみ）。

| 軸 | ラベル | 補足説明 |
|----|--------|----------|
| content(0-50) | コンテンツの充実度 | AIが引用したくなる情報の質・量 |
| technical(0-20) | AIが読みやすい構造 | Schema.org等でAIが正しく解釈できるか |
| authority(0-30) | 第三者からの評判・権威 | 他サイト・口コミでどれだけ言及されるか |

`AxisRow` に `description` を追加し、各軸名の下にキャプションで表示する。
