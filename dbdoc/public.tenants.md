...
| ---- | ---- | ---------- |
| fk_tenants_organization | FOREIGN KEY | FOREIGN KEY (organization_id) REFERENCES organizations(id) ON DELETE RESTRICT |
...
| idx_tenants_organization_active | CREATE INDEX idx_tenants_organization_active ON public.tenants USING btree (organization_id) WHERE (deleted_at IS NULL) |

## Triggers

| Name | Definition |
| ---- | ---------- |
| trg_tenants_updated_at | CREATE TRIGGER trg_tenants_updated_at BEFORE UPDATE ON public.tenants FOR EACH ROW EXECUTE FUNCTION update_updated_at_column() |
